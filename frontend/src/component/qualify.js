...
import data from '../data/sample_qualify.json';
...
const Chart = ({width, height, margin, data}) => {
    const hover = useContext(UserHoverContext);
    ...
    const chartRef = useD3(svg => {
        ...
        sumStat.forEach((group, key) => {
            ...
            svg.selectAll(`.dot-${key}`)
                .data(group)
                .join("circle")
                .attr("cx", d => xScale(parseFloat(d.Distance)))
                .attr("cy", d => yScale(parseInt(d.Speed)))
                .attr("r", 1.4)
